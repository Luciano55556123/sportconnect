...
final class Security
{
    ...
    public static function isHttps(array $config): bool
    {
        ...
        $forwardedSsl = strtolower(
            trim((string) ($_SERVER['HTTP_X_FORWARDED_SSL'] ?? ''))
        );

        $frontEndHttps = strtolower(
            trim((string) ($_SERVER['HTTP_FRONT_END_HTTPS'] ?? ''))
        );
        ...
        $trusted = array_filter(
            array_map(
                ...
                explode(
                    ',',
                    (string) ($config['trusted_proxies'] ?? '')
                )
            )
        );

        $trustAllProxies = in_array('*', $trusted, true);

        $isTrustedProxy = $trustAllProxies
            || (
                $remote !== ''
    ...
    public static function rateLimit(
        ...
        $_SESSION['rate_limits'][$key] = array_values(
            array_filter(
                $_SESSION['rate_limits'][$key] ?? [],
                static fn (int $time): bool =>
                    $time > ($now - $seconds)
            )
        );

        if (
            count($_SESSION['rate_limits'][$key]) >= $limit
        ) {
            error_log(
                'rate_limit action='
                . $action
    ...
    private static function expireSession(): void
    {
        ...
        if (
            $inactiveFor > self::INACTIVITY_SECONDS
            || $sessionAge > self::ABSOLUTE_SECONDS
        ) {
            Auth::logout();
